...
StabChainPlusLev<Telt, Tidx_label>
StructuralCopy(StabChainPlusLev<Telt, Tidx_label> const &S) {
...
// modified
template <typename Telt, typename Tidx_label>
permPlusBool<Telt> ExtendedT(Telt const &t, typename Telt::Tidx const &pnt,
                             typename Telt::Tidx img,
...
template <typename Tidx> struct singStrat {
  Tidx p;
  Tidx s;
  Tidx i;
};

template <typename Tidx> struct Refinement {
public:
  Refinement(Tidx const &val1, Tidx const &val2) {
    nature = 0;
    inputProcessfix = {val1, val2};
  }
  Refinement(Partition<Tidx> const &ePart,
             std::vector<singStrat<Tidx>> const &strat) {
    nature = 1;
...
template <typename Tidx> struct Trfm_centralizer {
  Tidx cellnum;
  Tidx g;
  Tidx img;
  Tidx strat;
...
template <typename Tidx> struct Trfm_processfixpoint {
  Tidx pnt;
  Tidx strat;
};

template <typename Tidx> struct Trfm_intersection {
  Partition<Tidx> Q;
...
bool IsInsertableRefinement(Trfm const &eRfm) {
  return std::visit(
      [&](auto &&arg) -> bool {
        static_assert(
            is_one_of<decltype(arg), Trfm_processfixpoint<Tidx>,
                      Trfm_intersection<Tidx>, Trfm_centralizer<Tidx>>{},
            "Non matching type.");
...
// Since we did not implement all the algorithms of stbcbckt, the value is
// always 0.
...
template <typename Telt> struct dataType_opperm {
...
//
// What is apparent in runs is that there is a link between rbase.lev[d] and
// rbase.level We have Add(rbase.lev, rbase.level) Later rbase.level is changed
// and the value of rbase.lev[d] accordingly as well. This proves the existence
// of a link which is otherwise hard to find explicitly.
//
...
//    strat := StratMeetPartition( rbase, P, O ); # Which also uses
//    ProcessFixpoint Therefore ProcessFixpoint seems to be the critical entry
...
//    This is because we have the increment    rbase.level :=
//    rbase.level.stabilizer;
// ---ChangeStabChain is very complicated. We have S:=G, G not modified but
//      G has been altered by the operation. But S is not the same as G in the
//      end. So, the operations are just very complicated to follow.
...
//          The operations S:=S.stabilizer are operations that makes S different
//          from G.
...
//    of pointers.
template <typename Telt, typename Tidx_label, typename Trfm> struct rbaseType {
  using Tidx = typename Telt::Tidx;
...
void KeyUpdatingRbase(std::string const &str,
  ...
  if (DoPrint) {
    ...
    bool test =
        ListKey[len - 1] == GetStringExpressionOfStabChain(rbase.level.Stot);
...
template <typename Telt, typename Tidx_label, typename Trfm>
std::string
ListOrbitOfRbaseLEV(rbaseType<Telt, Tidx_label, Trfm> const &rbase) {
  std::string str = "[ ";
...
template <typename Telt, typename Tidx_label, typename Trfm>
void PrintRBaseLevel(rbaseType<Telt, Tidx_label, Trfm> const &rbase,
  ...
  if (rbase.level.status == int_int) {
    ...
    if (rbase.level.status == int_stablev) {
      ...
      std::cerr << "CPP sgs(rbase.level)="
                ...
                << "\n";
      std::cerr << str << " PRBL rbase.level, record, |genlabels|="
                << rbase.level.Stot->genlabels.size() << "\n";
      std::cerr << str << " PRBL orbit=" << PrintTopOrbit(rbase.level.Stot)
                << "\n";
    } else {
      std::cerr << str
                << " PRBL rbase.level=" << GetIntTypeNature(rbase.level.status)
                << "\n";
    }
  }
}
...
template <typename Telt, typename Tidx_label, typename Tdata> struct imageType {
  size_t depth;
  ...
  StabChainPlusLev<Telt, Tidx_label> level2;
  imageType(Partition<typename Telt::Tidx> &_partition, Tdata &_data)
      : partition(_partition), data(_data) {}
};

template <typename Telt, typename Tidx_label, typename Tdata>
bool ProcessFixpoint_image(imageType<Telt, Tidx_label, Tdata> &image,
                           typename Telt::Tidx const &pnt,
                           typename Telt::Tidx &img,
                           typename Telt::Tidx const &simg) {
  using Tidx = typename Telt::Tidx;
...
#ifdef DEBUG_STBCBCKT
    std::cerr << "CPP PFI sgs(level2)="
              << GapStringTVector(
                     SortVector(StrongGeneratorsStabChain(image.level2.Stot)))
              << "\n";
    std::cerr << "CPP Case image.perm.status = false\n";
#endif
    permPlusBool<Telt> t =
        ExtendedT(image.perm2.val, pnt, img, std::numeric_limits<Tidx>::max(),
                  image.level2);
    if (t.status == int_false) {
#ifdef DEBUG_STBCBCKT
...
template <typename Telt, typename Tidx_label, typename Trfm>
rbaseType<Telt, Tidx_label, Trfm>
EmptyRBase(std::vector<StabChain<Telt, Tidx_label>> const &G, bool const &IsId,
           std::vector<typename Telt::Tidx> const &Omega,
           Partition<typename Telt::Tidx> const &P) {
  using Tidx = typename Telt::Tidx;
  rbaseType<Telt, Tidx_label, Trfm> rbase;
  rbase.domain = Omega;
  rbase.base = {};
...
bool MeetPartitionStrat(
    [[maybe_unused]] rbaseType<Telt, Tidx_label, Trfm> const &rbase,
    imageType<Telt, Tidx_label, Tdata> &image,
...
*/
template <typename Telt, typename Tidx_label, typename Trfm>
std::vector<singStrat<typename Telt::Tidx>> StratMeetPartition_r_p_p_e(
    rbaseType<Telt, Tidx_label, Trfm> &rbase, Partition<typename Telt::Tidx> &P,
    Partition<typename Telt::Tidx> const &S, Telt const &g) {
  using Tidx = typename Telt::Tidx;
  std::vector<singStrat<Tidx>> strat;
  ...
  for (Tidx s = 0; s < nrcells; s++) {
    ...
    for (auto &pVal : splits) {
      ...
      Tidx i = SplitCell_Partition_e(P, pVal, S, s, g,
                                     std::numeric_limits<Tidx>::max());
#ifdef DEBUG_STBCBCKT
...
template <typename Telt, typename Tidx_label, typename Trfm>
std::vector<singStrat<typename Telt::Tidx>>
StratMeetPartition_p_p_e(Partition<typename Telt::Tidx> &P,
...
template <typename Telt, typename Tidx_label, typename Trfm>
std::vector<singStrat<typename Telt::Tidx>>
StratMeetPartition_r_p_p(rbaseType<Telt, Tidx_label, Trfm> &rbase,
                         Partition<typename Telt::Tidx> &P,
                         Partition<typename Telt::Tidx> const &S) {
  using Tidx = typename Telt::Tidx;
  std::vector<singStrat<Tidx>> strat;
...
std::vector<singStrat<Tidx>> StratMeetPartition_p_p(Partition<Tidx> &P,
                                                    Partition<Tidx> const &S) {
  std::vector<singStrat<Tidx>> strat;
  std::vector<Tidx> cellsP = P.cellno;
  // If <S> is just a set, it is interpreted as partition ( <S>|<S>^compl ).
  Tidx nrcells = NumberCells(S) - 1;
...
void AddRefinement(rbaseType<Telt, Tidx_label, Trfm> &rbase, Trfm const &eRfm) {
  ...
  for (size_t i = 0; i < rbase.rfm.size(); i++) {
    std::cerr << "CPP i=" << int(i + 1)
              << " |rbase.rfm[i]|=" << rbase.rfm[i].size() << "\n";
  }
#endif
}

template <typename Telt, typename Tidx_label, typename Trfm>
void RegisterRBasePoint(Partition<typename Telt::Tidx> &P,
                        rbaseType<Telt, Tidx_label, Trfm> &rbase,
                        typename Telt::Tidx const &pnt) {
  using Tidx = typename Telt::Tidx;
  if (rbase.level2.status != int_true && rbase.level2.status != int_false) {
    ...
    std::cerr << "CPP rbase.level2.status="
  ...
  rbase.rfm.push_back({});
#ifdef DEBUG_STBCBCKT
  std::cerr << "CPP Before P.lengths test k=" << int(k + 1)
            << " len=" << rbase.rfm.size() << "\n";
...
#ifdef DEBUG_STBCBCKT
    std::cerr << "CPP Section P.lengths after FixpointCellNo pnt="
              << int(pnt + 1) << "\n";
...
#endif
    auto MainInsert =
        [&](StabChainPlusLev<Telt, Tidx_label> const &lev) -> void {
      if (lev.status != int_int) {
        ...
        std::cerr << "CPP StrongGeneratorsStabChain(lev) = "
                  << GapStringTVector(SortVector(LGenStrong)) << "\n";
#endif
...
#endif
        std::vector<singStrat<Tidx>> strat =
            StratMeetPartition_r_p_p(rbase, P, O);
#ifdef DEBUG_STBCBCKT
...
void NextRBasePoint_no_order(Partition<typename Telt::Tidx> &P,
  ...
  if (rbase.level.status == int_int) {
    ...
    while (true) {
      l = PositionProperty<Tidx>(
          ClosedInterval<Tidx>(0, lens[k]), [&](Tidx const &i) -> bool {
            return !IsFixedStabilizer(rbase.level.Stot,
                                      P.points[i + P.firsts[order[k]]]);
          });
      if (l != miss_val)
        break;
      k++;
...
template <typename Telt, typename Tidx_label, typename Trfm>
void NextRBasePoint_order(Partition<typename Telt::Tidx> &P,
                          rbaseType<Telt, Tidx_label, Trfm> &rbase,
  ...
  Tidx p = miss_val;
  if (rbase.level.status == int_int) {
    p = PositionProperty<Tidx>(order, [&](const Tidx &p_i) -> bool {
      return lens[CellNoPoint(P, p_i)] != 1;
    });
  } else {
    p = PositionProperty<Tidx>(order, [&](const Tidx &p_i) -> bool {
      return lens[CellNoPoint(P, p_i)] != 1 &&
             !IsFixedStabilizer(rbase.level.Stot, p_i);
    });
  }
  if (p != miss_val) {
    p = order[p];
...
template <typename Telt, typename Tidx_label, typename Tdata, typename Trfm>
bool Refinements_ProcessFixpoint(
    [[maybe_unused]] rbaseType<Telt, Tidx_label, Trfm> &rbase,
    imageType<Telt, Tidx_label, Tdata> &image, typename Telt::Tidx const &pnt,
    typename Telt::Tidx const &cellnum) {
  using Tidx = typename Telt::Tidx;
...
template <typename Telt, typename Tidx_label, typename Tdata, typename Trfm>
bool Refinements_Centralizer(
    [[maybe_unused]] rbaseType<Telt, Tidx_label, Trfm> &rbase,
    imageType<Telt, Tidx_label, Tdata> &image,
    const typename Telt::Tidx &cellnum, const typename Telt::Tidx &g,
    const typename Telt::Tidx &pnt, const typename Telt::Tidx &strat) {
  using Tidx = typename Telt::Tidx;
...
//
template <typename Telt, typename Tidx_label, typename Tdata, typename Trfm>
int RRefine(rbaseType<Telt, Tidx_label, Trfm> &rbase,
  ...
  auto Evaluation = [&](Trfm const &eRef) -> bool {
    return std::visit(
        [&](auto &&arg) -> bool {
          ...
          using T = std::decay_t<decltype(arg)>;
          if constexpr (std::is_same_v<T, Trfm_processfixpoint<Tidx>>)
            return Refinements_ProcessFixpoint(rbase, image, arg.pnt,
                                               arg.strat);
          else if constexpr (std::is_same_v<T, Trfm_intersection<Tidx>>)
            return Refinements_Intersection(rbase, image, arg.Q, arg.strat);
          else if constexpr (std::is_same_v<T, Trfm_centralizer<Tidx>>)
            return Refinements_Centralizer(rbase, image, arg.cellnum, arg.g,
                                           arg.img, arg.strat);
        },
        eRef);
  };
...
template <typename Telt, typename Tidx_label>
bool PBIsMinimal(std::vector<typename Telt::Tidx> const &range,
                 typename Telt::Tidx const &a, typename Telt::Tidx const &b,
                 StabChain<Telt, Tidx_label> const &S) {
  using Tidx = typename Telt::Tidx;
...
void SubtractBlistOrbitStabChain(Face &blist, std::vector<Telt> const &LGen,
                                 typename Telt::Tidx const &pnt_in) {
  using Tidx = typename Telt::Tidx;
...
template <typename Telt>
Telt MappingPermListList(typename Telt::Tidx const &n,
                         std::vector<typename Telt::Tidx> const &src,
                         std::vector<typename Telt::Tidx> const &dst) {
  using Tidx = typename Telt::Tidx;
...
template <typename Telt, typename Tidx_label, typename Tdata, typename Trfm,
          bool repr>
imageType<Telt, Tidx_label, Tdata>
BuildInitialImage(rbaseType<Telt, Tidx_label, Trfm> &rbase, Tdata &data) {
  if (repr) {
...
ResultPBT<Telt, Tidx_label> PartitionBacktrack(
    StabChain<Telt, Tidx_label> const &G, const F_pr &Pr, F_nextLevel nextLevel,
    rbaseType<Telt, Tidx_label, Trfm> &rbase, Tdata &data,
    StabChain<Telt, Tidx_label> &L, StabChain<Telt, Tidx_label> &R) {
  using Tidx = typename Telt::Tidx;
  Tidx n = G->comm->n;
  ...
  std::cerr << "CPP R=\n";
  PrintStabChain(R);
  std::cerr << "CPP INIT sgs(G)="
            << GapStringTVector(SortVector(StrongGeneratorsStabChain(G)))
            << "\n";
  std::cerr << "CPP INIT sgs(L)="
            << GapStringTVector(SortVector(StrongGeneratorsStabChain(L)))
            << "\n";
  std::cerr << "CPP INIT sgs(R)="
            << GapStringTVector(SortVector(StrongGeneratorsStabChain(R)))
            << "\n";
#endif
  imageType<Telt, Tidx_label, Tdata> image =
      BuildInitialImage<Telt, Tidx_label, Tdata, Trfm, repr>(rbase, data);
  std::vector<Face> orB; // backup of <orb>.
  std::vector<Face> orb;
  std::vector<std::vector<Tidx>>
      org; // intersected (mapped) basic orbits of <G>
  ...
  std::function<permPlusBool<Telt>(size_t const &, bool const &)> PBEnumerate =
...
#ifdef DEBUG_STBCBCKT
      std::cerr << "CPP PBEnumerate, EXIT 1, image.perm.status="
                << GetIntTypeNature(image.perm.status) << "\n";
#endif
      return {int_fail, {}};
    }
    image.depth = d;
...
#ifdef DEBUG_STBCBCKT
    std::cerr << "CPP PBEnumerate, step 4 d=" << int(d + 1)
              << " |rbase.base|=" << rbase.base.size() << "\n";
      ...
      if (IsTrivialRBase(rbase)) {
        ...
        if (wasTriv) {
          ...
          StabChainOptions<Tint, Telt> options =
              GetStandardOptions<Tint, Telt>(n);
          options.base = rbase.base;
          options.reduced = false;
#ifdef DEBUG_STBCBCKT
          std::cerr << "CPP Before computation of ListStabChain Order(L)="
                    << Order<Telt, Tidx_label, Tint>(L) << "\n";
          std::cerr << "CPP sgs(L)="
                    << GapStringTVector(
                           SortVector(StrongGeneratorsStabChain(L)))
        ...
        std::cerr << "CPP Before NextRBasePoint image.p.c="
                  << GapStringIntVector(image.partition.cellno) << "\n";
#endif
        nextLevel(rbase.partition, rbase, id);
#ifdef DEBUG_STBCBCKT
        std::cerr << "CPP After NextRBasePoint image.p.c="
                  << GapStringIntVector(image.partition.cellno) << "\n";
    ...
    if (image.perm.status == int_true) {
...
#endif
      AssignationVectorGapStyle(orb, d,
                                BlistList(range, Cell(oldcel, rbase.where[d])));
      if (image.level2.status != int_false) {
        b = orb[d].find_first();
        while (b != boost::dynamic_bitset<>::npos) {
          if (!IsInBasicOrbit(rbase.lev2[d].Stot,
                              SlashAct(Tidx(b), image.perm2.val)))
            orb[d][b] = false;
          b = orb[d].find_next(b);
        }
      }
#ifdef DEBUG_STBCBCKT
      std::cerr << "CPP ORB: Case image.perm=true d=" << int(d + 1)
                << " orb[d]=" << GetStringGAP(orb[d]) << "\n";
#endif
    } else {
#ifdef DEBUG_STBCBCKT
      ...
      std::cerr << "CPP ORB: Before pVal loop d=" << int(d + 1)
                << " orb[d]=" << GetStringGAP(orb[d]) << "\n";
      ...
      for (auto &pVal : rbase.lev[d].Stot->orbit) {
        ...
        std::cerr << "CPP pVal=" << int(pVal + 1) << " b=" << int(b + 1)
                  << "\n";
        std::cerr << "CPP oldcell=" << int(oldcel_cellno[b] + 1)
                  << " rbase.where=" << int(rbase.where[d] + 1) << "\n";
#endif
...
#ifdef DEBUG_STBCBCKT
      std::cerr << "CPP ORB: After pVal loop d=" << int(d + 1)
                << " orb[d]=" << GetStringGAP(orb[d]) << "\n";
#endif
    }
#ifdef DEBUG_STBCBCKT
...
#endif
    if (d == 0 && PML_ForAll(G->comm->labels, [&](Telt const &x) {
          return PowAct(a, x) == a;
        })) {
      orb[d][a] = true; // ensure a is a possible image (can happen if acting on
                        // permutations with more points)
#ifdef DEBUG_STBCBCKT
      std::cerr << "CPP ORB: After assignation d=" << int(d + 1)
                << " orb[d]=" << GetStringGAP(orb[d]) << "\n";
#endif
    }
    ...
    if (wasTriv) {
      ...
      std::cerr << "CPP Before RRefine 1 rbase.p.c="
                << GapStringIntVector(rbase.partition.cellno) << "\n";
      std::cerr << "CPP Before RRefine 1 image.p.c="
                << GapStringIntVector(image.partition.cellno) << "\n";
#endif
      RRefine(rbase, image, true);
#ifdef DEBUG_STBCBCKT
      std::cerr << "CPP After RRefine 1 image.p.c="
                << GapStringIntVector(image.partition.cellno) << "\n";
      ...
      // candidate list.
#ifdef DEBUG_STBCBCKT
      std::cerr << "CPP ORB 1: Before subtract d=" << int(d + 1)
                << " orb[d]=" << GetStringGAP(orb[d]) << "\n";
#endif
      SubtractBlist(orb[d], BlistList(range, L_list[d]->orbit));
#ifdef DEBUG_STBCBCKT
      std::cerr << "CPP ORB 1: After subtract d=" << int(d + 1)
                << " orb[d]=" << GetStringGAP(orb[d]) << "\n";
#endif
    }
#ifdef DEBUG_STBCBCKT
    ...
    if (wasTriv && a_size > max) {
      m--;
#ifdef DEBUG_STBCBCKT
      std::cerr << "CPP Before test m=" << m
                << " Length(L[d].orbit)=" << L_list[d]->orbit.size() << "\n";
#endif
      if (m < L_list[d]->orbit.size()) {
#ifdef DEBUG_STBCBCKT
    ...
    while (b != boost::dynamic_bitset<>::npos) {
      Tidx b_int = Tidx(b);
#ifdef DEBUG_STBCBCKT
      std::cerr << "CPP b=" << int(b + 1) << " b_int=" << int(b_int + 1)
                << " d=" << int(d + 1) << "\n";
      ...
      if (!repr && !wasTriv && R_list[d]->orbit.size() > 0) {
        ...
        while (dd < d) {
          ...
          std::cerr << "CPP while dd=" << int(dd + 1) << " d=" << int(d + 1)
                    << "\n";
#endif
          if (IsInBasicOrbit(L_list[dd], a) &&
              !PBIsMinimal(range, R_list[dd]->orbit[0], b_int, R_list[d])) {
            dd = d + 1;
#ifdef DEBUG_STBCBCKT
      ...
      if (dd == d) {
#ifdef DEBUG_STBCBCKT
        std::cerr << "CPP equality dd=d undoto=" << undoto
                  << " |image.partition|=" << NumberCells(image.partition)
                  << "\n";
#endif
...
#ifdef DEBUG_STBCBCKT
          std::cerr << "CPP Before UndoRefinement cellno="
                    << GapStringIntVector(image.partition.cellno) << " i=" << i
                    << "\n";
#endif
          UndoRefinement(image.partition);
#ifdef DEBUG_STBCBCKT
          std::cerr << "CPP After UndoRefinement cellno="
                    << GapStringIntVector(image.partition.cellno) << " i=" << i
                    << "\n";
#endif
        }
        ...
        if (val) {
#ifdef DEBUG_STBCBCKT
          std::cerr << "CPP Before RRefine 2 oldcel="
                    << GapStringIntVector(image.partition.cellno) << "\n";
#endif
          t.status = RRefine(rbase, image, false);
#ifdef DEBUG_STBCBCKT
          std::cerr << "CPP After RRefine 2 oldcel="
                    << GapStringIntVector(image.partition.cellno) << "\n";
#endif
        } else {
#ifdef DEBUG_STBCBCKT
...
#ifdef DEBUG_STBCBCKT
        std::cerr << "CPP After assignment of t. t.status="
                  << GapStringTrueFalseFail(t.status) << "\n";
#endif
        ...
        if (t.status != int_fail) {
          ...
          std::cerr << "CPP R[d]=\n";
          PrintStabChain(R_list[d]);
          std::cerr << "CPP IsIdenticalObj="
                    << IsIdenticalObj(L_list[d], R_list[d]) << "\n";
#endif
          ...
          if (2 * (d + 1) <= blen) {
            ...
            std::vector<Telt> LGenB =
                PML_Filtered(LGen, [&](Telt const &gen) -> bool {
                  return PowAct(b_int, gen) == b_int;
                });
#ifdef DEBUG_STBCBCKT
        ...
        // candidate list.
#ifdef DEBUG_STBCBCKT
        std::cerr << "CPP ORB 2: Before subtract d=" << int(d + 1)
                  << " orb[d]=" << GetStringGAP(orb[d]) << "\n";
#endif
        if (R_list[d]->transversal.size() > 0 &&
            R_list[d]->transversal[b] !=
          ...
          SubtractBlistOrbitStabChain(
              orb[d], StrongGeneratorsStabChain(R_list[d]), b_int);
        }
#ifdef DEBUG_STBCBCKT
        std::cerr << "CPP ORB 2: After subtract d=" << int(d + 1)
                  << " orb[d]=" << GetStringGAP(orb[d]) << "\n";
#endif
      }
      b = orb[d].find_next(b);
...
#ifdef DEBUG_STBCBCKT
        std::cerr << "CPP ProcessFixpoint_image, Case PartitionBacktrack 2 i="
                  << int(i + 1) << " fix=" << int(fix[i] + 1)
                  << " fixP=" << int(fixP[i] + 1) << "\n";
#endif
...
template <typename Telt> Face OnSets(Face const &f, Telt const &g) {
  using Tidx = typename Telt::Tidx;
  size_t n = f.size();
  Face fRet(n);
...
ResultPBT<Telt, Tidx_label>
RepOpSetsPermGroup(StabChain<Telt, Tidx_label> const &G, Face const &Phi,
                   Face const &Psi) {
  using Tidx = typename Telt::Tidx;
#ifdef DEBUG_STBCBCKT
  ...
  if (repr && Phi.size() != Psi.size())
    return {int_fail, {}, {}};
  if (IsSubset(Phi, Omega) ||
      PML_ForAll(Omega, [&](Tidx const &p) -> bool { return !Phi[p]; })) {
    if (repr) {
    ...
    if (repr &&
        (IsSubset(Psi, Omega) ||
         PML_ForAll(Omega, [&](Tidx const &p) -> bool { return !Psi[p]; })))
      return {int_fail, {}, {}};
  }
  auto GetPartitionFromPair = [&](Face const &Ph) -> Partition<Tidx> {
    ...
    //    std::cerr << "CPP IntVect=" << GapStringIntVector(IntVect) << "
    //    DiffVect=" << GapStringIntVector(DiffVect) << "\n";
  ...
  auto GetSubgroup = [&](Face const &Ph) -> StabChain<Telt, Tidx_label> {
    std::vector<Telt> sgs = PML_Filtered(
        LGen, [&](Telt const &g) -> bool { return OnSets(Ph, g) == Ph; });
#ifdef DEBUG_STBCBCKT
  ...
  if (repr)
    ...
    R = L;
#ifdef DEBUG_STBCBCKT
  std::cerr << "CPP Orders: |R|=" << SizeStabChain<Telt, Tidx_label, Tint>(R)
            << " |L|=" << SizeStabChain<Telt, Tidx_label, Tint>(L) << "\n";
#endif
...
Kernel_Stabilizer_OnPoints(StabChain<Telt, Tidx_label> const &G,
...
#ifdef DEBUG_STBCBCKT
    std::cerr << "CPP orbit[1]=" << int(Sptr->orbit[0] + 1)
              << " x=" << int(x + 1) << "\n";
#endif
    if (Sptr->orbit[0] != x) {
#ifdef DEBUG_STBCBCKT
...
template <typename Telt, typename Tidx_label, typename Tint>
StabChain<Telt, Tidx_label>
Kernel_Stabilizer_OnSets(StabChain<Telt, Tidx_label> const &G,
                         Face const &Phi) {
  using Tidx = typename Telt::Tidx;
#ifdef DEBUG_STBCBCKT
  std::cerr << "CPP Beginning of Stabilizer_OnSets\n";
  ...
  if (2 * Phi.count() > n) {
    ...
    for (size_t i = 0; i < n; i++)
      PhiC[i] = 1 - Phi[i];
    if (PhiC.count() > 1) {
      return RepOpSetsPermGroup<Telt, Tidx_label, Tint, false>(G, PhiC, PhiC)
          .stab;
    }
    if (PhiC.count() == 0) {
      return G;
    ...
    if (Phi.count() > 1) {
      return RepOpSetsPermGroup<Telt, Tidx_label, Tint, false>(G, Phi, Phi)
          .stab;
    }
    if (Phi.count() == 0) {
      return G;
...
template <typename Telt, typename Tidx_label, typename Tint>
StabChain<Telt, Tidx_label>
Kernel_Stabilizer_OnPoints_backtrack(StabChain<Telt, Tidx_label> const &G,
                                     typename Telt::Tidx const &x) {
  using Tidx = typename Telt::Tidx;
#ifdef DEBUG_STBCBCKT
  std::cerr << "CPP Beginning of Stabilizer_OnSets\n";
...
template <typename Telt, typename Tidx_label, typename Tint>
std::optional<Telt>
Kernel_RepresentativeAction_OnSets(StabChain<Telt, Tidx_label> const &G,
  ...
  auto Process_ResultPBT =
      [&](ResultPBT<Telt, Tidx_label> const &eRec) -> std::optional<Telt> {
  ...
  if (2 * f1.count() > n) {
    ...
    ResultPBT<Telt, Tidx_label> eRec =
        RepOpSetsPermGroup<Telt, Tidx_label, Tint, true>(G, f1C, f2C);
    return Process_ResultPBT(eRec);
  } else {
    ResultPBT<Telt, Tidx_label> eRec =
        RepOpSetsPermGroup<Telt, Tidx_label, Tint, true>(G, f1, f2);
    return Process_ResultPBT(eRec);
  }
}
...
Kernel_RepresentativeAction_OnPoints(StabChain<Telt, Tidx_label> const &G,
                                     typename Telt::Tidx const &x1,
                                     typename Telt::Tidx const &x2) {
  using Tidx = typename Telt::Tidx;
  Tidx n = G->comm->n;
#ifdef PERMUTALIB_BLOCKING_SANITY_CHECK
  if (x1 >= n || x2 >= n) {
    std::cerr << "We should have x1 <n && x2 < n. x1=" << x1 << " x2=" << x2
              << " n=" << n << "\n";
    throw PermutalibException{1};
  }
#endif
...
Cycles(const Telt &x, const std::vector<typename Telt::Tidx> &Omega) {
  using Tidx = typename Telt::Tidx;
  Tidx n = x.size();
...
template <typename Telt, typename Tidx_label, typename Tint, bool repr>
ResultPBT<Telt, Tidx_label>
RepOpElmTuplesPermGroup(const StabChain<Telt, Tidx_label> &G,
  ...
  if (PML_ForAll(Kernel_GeneratorsOfGroup(G),
                 [&](const Telt &gen) -> bool { return test_gen(gen); })) {
    if (!repr) {
      return {int_group, G, {}};
    } else {
      if (e != f)
    ...
    for (size_t i_pnt = 0; i_pnt < fix.size();
      ...
      for (Tidx g = 0; g < e_siz_i; g++) {
        ...
        if (strat != miss_val) {
          ...
          AddRefinement(rbase, Trfm(Trfm_centralizer<Tidx>(
  ...
  auto get_Q = [&]() -> Partition<Tidx> {
    ...
    for (size_t i = 0; i < e_siz; i++) {
      StratMeetPartition_p_p(
          Q, CollectedPartition(GetPartition(Cycles(f[i], Omega)), 1));
    }
    return Q;
  };
...
Kernel_Centralizer_elt(const StabChain<Telt, Tidx_label> &G, const Telt &e) {
  ...
  StabChain<Telt, Tidx_label> LR_grp =
      StabChainOp_listgen<Telt, Tidx_label, Tint>(LGen, options);
  return RepOpElmTuplesPermGroup<Telt, Tidx_label, Tint, false>(G, e_v, e_v,
                                                                LR_grp, LR_grp)
      .stab;
}
...
template <typename Telt, typename Tidx_label, typename Tint>
StabChain<Telt, Tidx_label>
Kernel_Centralizer_grp(const StabChain<Telt, Tidx_label> &G,
  ...
  StabChain<Telt, Tidx_label> LR_grp =
      StabChainOp_listgen<Telt, Tidx_label, Tint>(LGen, options);
  return RepOpElmTuplesPermGroup<Telt, Tidx_label, Tint, false>(
...
##
template<typename Telt, typename Tidx_label, typename Tint>
std::optional<Telt> ConjugatorPermGroup(const StabChain<Telt,Tidx_label>&G,
const StabChain<Telt,Tidx_label>& E, const StabChain<Telt,Tidx_label>& F)
{
    ...
    while Length(Omega)>0
      ...
      if i<=Length(orb) then
        ...
        if Size(Nh)<Size(Gh) then
          ...
          G:=PreImage(hom,Nh);
          Info(InfoGroup,1,"Orbit ",Length(so)," reduces by
",Size(Gh)/Size(Nh)); fi; Omega:=Difference(Omega,so); fi; od;
