...
template <typename T> std::vector<T> Reversed(std::vector<T> const &eList) {
...
boost::dynamic_bitset<>::size_type PositionNthTrueBlist(Face const &blist,
                                                        size_t const &hpos) {
...
template <typename Tidx>
std::vector<Tidx> ListBlist(std::vector<Tidx> const &list,
                            std::vector<int8_t> const &blist) {
  std::vector<Tidx> ret;
  size_t len = list.size();
...
size_t SizeBlist(Face const &blist) { return blist.count(); }
...
void IntersectBlist(Face &a, Face const &b) { a &= b; }
