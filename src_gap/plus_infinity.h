...
template <typename T> struct Tplusinfinity {
...
template <typename T> Tplusinfinity<T> GetInfinity() {
  T val = 0;
...
template <typename T> bool operator<=(T const &x, Tplusinfinity<T> const &y) {
  if (y.IsInfinity())
    return true;
  return x <= y.GetValue();
}
