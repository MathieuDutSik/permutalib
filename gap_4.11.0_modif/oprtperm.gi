...
InstallOtherMethod( RepresentativeActionOp, "permgrp",true, [ IsPermGroup,
  ...
  {} -> 2*RankFilter(IsMultiplicativeElementWithInverse),
    ...
    local   rep,                # representative, result
	    ...
	    dp,ep,		# point copies
            i,  f;              # loop variables
    Print("RepresentativeActionOp for permgrp\n");
    # standard action on points, make a basechange and trace the rep
    if act = OnPoints and IsInt( d ) and IsInt( e )  then
        Print("RepresentativeActionOp permgrp : case 1\n");
        d := [ d ];  e := [ e ];
        S := true;
    elif     ( act = OnPairs or act = OnTuples )
         and IsPositionsList( d ) and IsPositionsList( e )  then
        Print("RepresentativeActionOp permgrp : case 2\n");
        S := true;
    fi;
    if IsBound( S )  then
        Print("RepresentativeActionOp permgrp : case 3\n");
        if d = e  then
            Print("RepresentativeActionOp permgrp : case 3, step 1\n");
            rep := One( G );
        elif Length( d ) <> Length( e ) then
            Print("RepresentativeActionOp permgrp : case 3, step 2\n");
            rep:= fail;
        else
            Print("RepresentativeActionOp permgrp : case 3, step 3\n");
	    ...
	    if Length(sel)>0 then
	      ...
	      rep:=fail; # we did not yet get anything
            ...
            Print("RepresentativeActionOp permgrp : case 3, step 4, rep=", rep, "\n");
	    if rep=fail then
	      ...
	      od;
            ...
            Print("RepresentativeActionOp permgrp : case 3, step 5\n");

        fi;

    # action on (lists of) permutations, use backtrack
    elif act = OnPoints and IsPerm( d ) and IsPerm( e )  then
        Print("RepresentativeActionOp permgrp : case 4\n");
    ...
    # action on sets of points, use backtrack
        ...
        Print("RepresentativeActionOp permgrp : case 8\n");
    ...
    # action on tuples of sets
      ...
      and IsList(e) and ForAll(e,i->ForAll(i,IsInt)) then
        Print("RepresentativeActionOp permgrp : case 9\n");
    ...
    # other action, fall back on default representative
        ...
        Print("RepresentativeActionOp permgrp : case 10\n");
        TryNextMethod();
    fi;
